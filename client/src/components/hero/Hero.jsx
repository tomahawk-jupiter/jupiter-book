...
const Hero = () => {
  ...
  const handleFriendRequest = async () => {
    try {
      if (friendStatus === "add") {
        await axiosInstance.put(`/users/${profileId}/friend-request`, {
          userId: user._id,
        });
        setFriendStatus("cancel");
      }
      if (friendStatus === "accept") {
        await axiosInstance.put(`/users/${profileId}/accept`, {
          userId: user._id,
        });
        setFriendStatus("unfriend");
      }
      if (friendStatus === "cancel") {
        await axiosInstance.put(`/users/${profileId}/cancel`, {
          userId: user._id,
        });
        setFriendStatus("add");
      }
      if (friendStatus === "unfriend") {
        await axiosInstance.put(`/users/${profileId}/unfriend`, {
          userId: user._id,
        });
        setFriendStatus("add");
      }
