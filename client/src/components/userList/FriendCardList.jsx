...
const FriendCardList = ({ title }) => {
  ...
  const fetchFriends = async () => {
    try {
      let res = null;
      if (title === "Friends") {
        res = await axiosInstance.get(`/users/${user._id}/friends`);
        setFriendStatus("unfriend");
      }
      if (title === "Suggested") {
        res = await axiosInstance.get(`/users/suggested/${user._id}`);
        setFriendStatus("add");
      }
      if (title === "Requests") {
        res = await axiosInstance.get(`/users/${user._id}/received-requests`);
        setFriendStatus("accept");
      }
      if (title === "Pending") {
        res = await axiosInstance.get(`/users/${user._id}/sent-requests`);
  ...
  return (
    <>
      <div className="friendCardListComponent">
        ...
        {friends.length === 0 && <div className="noRequestsMessage">None</div>}
        {friends.map((friend) => {
          return (
            <FriendCard
              key={friend._id}
